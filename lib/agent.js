...
var Agent = function(){
				...
				if(error){
					...
					Court.getClosest(location.lng, location.lat, radius, function(error, courts){
						if(error){
							...
							self.startBooking(sender_id, postcode, courts, callback);
						}
					});
				}
			});			
	...
	this.getNearestCourtFromLocation = function(sender_id, coords, radius, callback){
		...
		Court.getClosest(coords.lng, coords.lat, radius, function(error, courts){
			if(error){
				callback(error, []);
			} else {
				self.startBooking(sender_id, "", courts, callback);
			}
		});
	}
			...
			if(error){
				...
				booking.save(function(error){
					if(error){
						...
						this.saveUser(sender_id, postcode, courts, callback);
